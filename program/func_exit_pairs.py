...
async def manage_trade_exits(client):
    ...
    for position in open_positions_dict:
        ...
        position_size_m1 = float(position["order_m1_size"])  # Convert size to float
        position_side_m1 = position["order_m1_side"]

        # Extract position matching information from file - market 2
        position_market_m2 = position["market_2"]
        position_size_m2 = float(position["order_m2_size"])  # Convert size to float
        ...
        order_size_m1 = float(order_m1["size"])  # Ensure the size is a float
        ...
        order_size_m2 = float(order_m2["size"])  # Ensure the size is a float
        ...
        if not check_m1 or not check_m2 or not check_live:
            ...
            print(f"This means that the program does not recognize some of the open positions. Please check and close trades for this pair.")
